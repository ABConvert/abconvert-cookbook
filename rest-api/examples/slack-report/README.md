...
4. Sends the figures to Claude for a short summary, then posts the summary to a Slack incoming webhook. The system prompt in `summarize()` sets the rules for `srm_status`, `outcome`, intervals, and money formatting.
...
node rest-api/examples/slack-report/report.mjs
```
