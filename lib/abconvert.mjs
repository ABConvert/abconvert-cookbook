 ...
 * wherever it is.
 *
 * `difference` is always statable, and `frequentist.difference_interval` bounds
 * it in the metric's own unit. So: quote the lift when there is one, and fall
 * back to the difference when there is not, rather than printing "n/a" over a
 * result that exists.
 */
