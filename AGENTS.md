...
- `browser-api/`: JavaScript API examples, storefront scripts that read `window.ABConvert`. Each is one plain script you add to a theme. `browser-api/playground/` runs them against a mock; serve it with `python3 -m http.server` from `browser-api/`. Contract: https://docs.abconvert.io/api-reference/browser-api (also at `.md` for agents).
