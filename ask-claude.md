...
- Changing a `split` on a running test only affects new visitors. Everyone already assigned keeps their test group, so the traffic already recorded still reflects the old split.
